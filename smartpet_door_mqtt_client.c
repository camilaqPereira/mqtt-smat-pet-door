 /* SMART PET DOOR - MQTT CLIENT
 *
 * DESCRIÇÃO: Este arquivo implementa o controle do acesso sa saída de um pet, utilizando as informações 
 * recebidas de um broker MQTT. O cliente MQTT conecta-se ao Wifi.
 * 
 * OBSERVAÇÃO: Este código é uma adaptação do exemplo 'AULA IoT - Cliente MQTT - Publisher:/Temperatura; Subscribed:/led'
 * desenvolvido por Ricardo Prates. O código original encontra-se disponível em: https://github.com/rmprates84/mqtt_client
 */
...
#include "hardware/pwm.h"           // Biblioteca de hardware para conversão ADC

#include "lib/hc_sr04.h"            // Biblioteca para o sensor ultrassônico HC-SR04
#include "lib/servomotor.h"         // Biblioteca para controle de servomotores
...
#define MQTT_SERVER "BROKER_IP"                // Substitua pelo endereço do host - broket MQTT: Ex: 192.168.1.107
#define MQTT_USERNAME "SEU_USERNAME"     // Substitua pelo nome da host MQTT - Username
#define MQTT_PASSWORD "SENHA_BROKER"     // Substitua pelo Password da host MQTT - credencial de acesso - caso exista


/*---------------------------------------------- MQTT AND WIFI DATA CONFIGURATION ------------------------------------------------*/
...
// Temporização da rotina do controlador da porta - em segundos
...
static async_at_time_worker_t controller_worker = { .do_work = controller_worker_fn};
...
static void sub_unsub_topics(MQTT_CLIENT_DATA_T* state, bool sub) {
    ...
    mqtt_sub_unsub(state->mqtt_client_inst, full_topic(state, "/tag/one"), MQTT_SUBSCRIBE_QOS, cb, state, sub);
    mqtt_sub_unsub(state->mqtt_client_inst, full_topic(state, "/tag/two"), MQTT_SUBSCRIBE_QOS, cb, state, sub);
    mqtt_sub_unsub(state->mqtt_client_inst, full_topic(state, "/tag/three"), MQTT_SUBSCRIBE_QOS, cb, state, sub);
...
static void mqtt_incoming_data_cb(void *arg, const u8_t *data, u16_t len, u8_t flags) {
    ...
    if (strcmp(basic_topic, "/door") == 0) {       
        ...
        if (lwip_stricmp((const char *)state->data, "Lock") == 0){
            door_locked = true; // Trancar porta
            
            const char *door_status_key = full_topic(state, "/door/status");
            const char *payload = "Trancada\0";
            mqtt_publish(state->mqtt_client_inst, door_status_key, payload , strlen(payload), MQTT_PUBLISH_QOS, MQTT_PUBLISH_RETAIN, pub_request_cb, state);
    
        }else if (lwip_stricmp((const char *)state->data, "Unlock") == 0){
            door_locked = false; // Destrancar porta

            const char *door_status_key = full_topic(state, "/door/status");
            const char *payload = "Destrancada\0";
            mqtt_publish(state->mqtt_client_inst, door_status_key, payload , strlen(payload), MQTT_PUBLISH_QOS, MQTT_PUBLISH_RETAIN, pub_request_cb, state);
        }
        
    } else if(strcmp(basic_topic, "/tag/one") == 0){       
        
        if (lwip_stricmp((const char *)state->data, "true") == 0){
            tag_permission[0] = true; // Autorizar pet 1

            const char *tag_status_key = full_topic(state, "/tag/one/status");
            const char *payload = "Autorizado\0";
            mqtt_publish(state->mqtt_client_inst, tag_status_key, payload , strlen(payload), MQTT_PUBLISH_QOS, MQTT_PUBLISH_RETAIN, pub_request_cb, state);
        }else if (lwip_stricmp((const char *)state->data, "false") == 0){
            tag_permission[0] = false; // Desautorizar pet 1
            
            const char *tag_status_key = full_topic(state, "/tag/one/status");
            const char *payload = "Não autorizado\0";
            mqtt_publish(state->mqtt_client_inst, tag_status_key, payload , strlen(payload), MQTT_PUBLISH_QOS, MQTT_PUBLISH_RETAIN, pub_request_cb, state);
        }
        
    } else if(strcmp(basic_topic, "/tag/two") == 0){
        if (lwip_stricmp((const char *)state->data, "true") == 0){
            tag_permission[1] = true; // Autorizar pet 2

            const char *tag_status_key = full_topic(state, "/tag/two/status");
            const char *payload = "Autorizado\0";
            mqtt_publish(state->mqtt_client_inst, tag_status_key, payload , strlen(payload), MQTT_PUBLISH_QOS, MQTT_PUBLISH_RETAIN, pub_request_cb, state);
        }else if (lwip_stricmp((const char *)state->data, "false") == 0){
            tag_permission[1] = false; // Desautorizar pet 2
    ...
    } else if(strcmp(basic_topic, "/tag/three") == 0){
        if (lwip_stricmp((const char *)state->data, "true") == 0){
            tag_permission[2] = true; // Autorizar pet 3

            const char *tag_status_key = full_topic(state, "/tag/three/status");
            const char *payload = "Autorizado\0";
            mqtt_publish(state->mqtt_client_inst, tag_status_key, payload , strlen(payload), MQTT_PUBLISH_QOS, MQTT_PUBLISH_RETAIN, pub_request_cb, state);
        }else if (lwip_stricmp((const char *)state->data, "false") == 0){
            tag_permission[2] = false; // Desautorizar pet 3
...
static void mqtt_connection_cb(mqtt_client_t *client, void *arg, mqtt_connection_status_t status) {
    ...
    if (status == MQTT_CONNECT_ACCEPTED) {
        ...
        controller_worker.user_data = state;
        async_context_add_at_time_worker_in_ms(cyw43_arch_async_context(), &controller_worker, 0);
